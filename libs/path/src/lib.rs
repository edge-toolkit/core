...
//!
//! Root-finding has two entry points: [`find_project_root`] (from an explicit start, used by the runtime
//! `edge_toolkit::config::get_project_root`) and [`find_project_root_from_manifest`] (from `CARGO_MANIFEST_DIR`, used
//! by build scripts). The path builders [`absolute_from`] and [`relative_path_from`] render POSIX-style paths for
//! `mise.toml` / `docker-compose.yaml` generation.
...
///
/// `.dprint.jsonc` is present at the root (and nowhere above it), so it is a reliable anchor.
const ROOT_MARKER: &str = ".dprint.jsonc";

/// Walk up from `start` to the first ancestor that contains `.dprint.jsonc`.
///
/// Falls back to `start` itself when no ancestor has the marker, mirroring the runtime helper's "use what we have"
/// behaviour.
#[must_use]
...
///
/// The result is rendered as a POSIX string regardless of host OS, because every caller writes it into generated
/// `mise.toml` / `docker-compose.yaml` output -- both of which expect forward-slash separators even on Windows.
#[must_use]
