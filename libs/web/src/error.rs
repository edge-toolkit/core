...
///
/// Works on any `Result<T, E>` where `E: std::fmt::Debug`. Use
/// `.js_context("ctx")` to attach a prefix to a JS error you propagated from
/// a fallible call.
...
/// `.into_promise("op")` shortcut for the WASM modules' recurring pattern.
///
/// Equivalent to `.dyn_into_msg::<Promise>("op did not return a Promise")`.
...
impl<S: JsCast> JsPromiseExt for S {
    fn into_promise(self, op: &str) -> Result<js_sys::Promise, JsValue> {
        self.dyn_into::<js_sys::Promise>()
            .map_err(|_original| JsValue::from_str(&format!("{op} did not return a Promise")))
    }
}

/// `.into_function("op")` shortcut for the WASM modules' recurring pattern.
///
/// Equivalent to `.dyn_into_msg::<Function>("op is not callable")`. Pass the
...
impl<S: JsCast> JsFunctionExt for S {
    fn into_function(self, op: &str) -> Result<js_sys::Function, JsValue> {
        self.dyn_into::<js_sys::Function>()
            .map_err(|_original| JsValue::from_str(&format!("{op} is not callable")))
    }
}
