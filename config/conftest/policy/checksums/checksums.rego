...
#
# Forward: a `*_asset` var must have a matching entry recorded, or its
# fetch-* task would download something we can't integrity-verify.
# Reverse: a `[asset.<filename>]` table must be referenced by a
# `*_asset` var, or it is a stale record from a refresh someone forgot
# to wire all the way through.
#
# Each `[asset.<filename>]` table must carry `url` (the canonical
# download URL), `upstream` (project/dataset the asset was built from),
# and `license` (SPDX expression). `sha256` may be empty (`""`) while
# we bootstrap a new upstream-cache release before the first upload --
