...
/// The volatile ` on <YYYY-MM-DD HH:MM:SS> UTC` suffix openapi2zig stamps into its `// <auto-generated>` header.
///
/// It is a wall-clock time, so the committed client would differ on every regen and fail `gen-specs-check`. We
/// strip just this suffix (keeping the stable `openapi2zig <version> (<commit>)` identity) so the output is
