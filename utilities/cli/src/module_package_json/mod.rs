...
use crate::error::{CliError, IoOp, IoResultExt, parse_json, parse_toml, serialize_json_pretty};
...
pub fn generate_module_package_json(module_dir: &Path) -> Result<PathBuf, CliError> {
    ...
    fs::create_dir_all(parent).io_context(IoOp::CreateDir, parent)?;
    let mut out = serialize_json_pretty(&package_json)?;
    out.push('\n');
    fs::write(&out_path, &out).io_context(IoOp::Write, &out_path)?;
...
fn package_json_from_cargo(module_dir: &Path, out_path: &Path) -> Result<Value, CliError> {
    ...
    let cargo_toml_src = fs::read_to_string(&cargo_toml_path).io_context(IoOp::Read, &cargo_toml_path)?;
...
{
    let src = fs::read_to_string(path).io_context(IoOp::Read, path)?;
    parse_toml(path, &src)
}
