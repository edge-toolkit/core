...
async fn echo_module_round_trips() -> Result<(), Box<dyn Error>> {
    ...
    let echo_result = tokio::time::timeout(
        ...
        echo_round_trip(&mut control, payload, &control_id),
    )
    .await;
    ...
    let observed = echo_result??;
    if observed != payload {
        return Err(format!("echo response {observed:?} did not match {payload:?}").into());
    }
    Ok(())
}
...
async fn echo_round_trip(control: &mut ControlSocket, payload: &str, self_id: &str) -> Result<String, Box<dyn Error>> {
