...
//!
//! Starts the same hub the integration tests use on a fixed port (the one the browser wasm-agent tests connect to),
//! writes a readiness marker to the file named by the first CLI argument once the server is accepting, then stays up
//! until the task deletes that marker -- its stop signal. Returning from `main` (rather than being killed) lets the
//! coverage-instrumented build flush its counters on a clean exit; a SIGKILL would drop them.
//!
//! Excluded from Codacy analysis (`.codacy.yaml`): Codacy's Rust security rule flags `args_os()` flowing into a file
...
/// Fixed port the browser wasm-agent tests dial (`ws://127.0.0.1:8080/ws`).
///
/// Matches the ws-server port the existing `web.rs` end-to-end test and `ws-e2e-chrome` use.
