...
/// Build a `WasiNnCtx` configured with whatever backends the crate's feature
/// flags enabled (just `onnx` for us). Empty registry — guests load model
/// bytes directly via `graph.load`, so name-based lookup isn't needed.
